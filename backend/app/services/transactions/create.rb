...
module Transactions
  ...
  class Create
    ...
    # @raises [ApplicationError] When validation fails
    ...
    def call(from_currency:, to_currency:, from_value:)
      validate_params!(from_currency, to_currency, from_value)
    ...
    def validate_params!(from_currency, to_currency, from_value)
      errors = []
    ...
    def fetch_exchange_rate(from_currency, to_currency)
      ...
      @exchange_rate_provider.fetch_rate(
        from: from_currency,
        to: to_currency,
      ).tap { |rate| Rails.logger.info("Exchange rate fetched: #{rate}") }
    end

    def calculate_converted_value(from_value, rate)
      # Convert to BigDecimal and calculate, rounding to 4 decimal places
      (BigDecimal(from_value.to_s) * rate).round(4)
    end
