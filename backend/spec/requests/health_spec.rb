...
RSpec.describe 'Health Check' do
  describe 'GET /api/v1/health' do
    ...
    it 'reports service failures' do
      ...
      get '/api/v1/health'
      expect(response.parsed_body['services']['database']).to include('status' => 'down', 'message' => 'DB error')

      allow(ActiveRecord::Base.connection).to receive(:execute).and_call_original
      allow(Rails.cache).to receive(:write).and_raise(StandardError.new('Cache error'))
      get '/api/v1/health'
      expect(response.parsed_body['services']['cache']).to include('status' => 'down', 'message' => 'Cache error')
    end
  end
end
